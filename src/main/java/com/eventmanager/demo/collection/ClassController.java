...
public class ClassController {
    ...
    ConsultList<ClassCollection> getClasses(@RequestParam(value = "resource", required = false) Integer resource, @RequestParam(value = "pagination", required = false) String pagination, @RequestParam(value = "notInResource", required = false) Integer notInResource) throws JsonProcessingException {
        ...
        if (resource != null) {
            ...
            List<Integer> notTheseClasses = classRepository.findIdByResourcesId(notInResource).stream().map(ClassCollection::getId).collect(Collectors.toList());
            if(notTheseClasses.size() > 0) {
                classes = classRepository.findByIdNotIn(notTheseClasses, pageRequest);
                count = classRepository.countByIdNotIn(notTheseClasses);
            } else {
